...
class BayesianBasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out, kl_lwta = self.lwta_activation(out, self.temperature)
        kl_total += kl_lwta
        return out, kl_total
    ...
    def lwta_activation(self, input, temp = 0.67, hard = False):
        ...
        # logits = torch.reshape(input, [-1, input.size(-1), self.K, self.U])
...
class BayesianSparseNeuralSurvival(nn.Module):
    def __init__(self, input_dim, layers, output_dim=1, U = 2, temperature = 1.67, prior_var=1.0, risks=1):
        super(BayesianSparseNeuralSurvival, self).__init__()
    ...
    def forward(self, x, horizon, gradient=False):
        ...
        tau_outcome = horizon.clone().detach().requires_grad_(gradient).unsqueeze(1)

...
from BNN_Sparse_Monotonic.model import BayesianSparseNeuralSurvivalTorch
from BNN_Sparse_Monotonic import Losses
from BNN_Sparse_Monotonic.utilities import train_model
...
class BayesianSparseNeuralSurvival(DSMBase):
  ...
  def fit(self, x, t, e, vsize = 0.2, val_data = None,
          optimizer = "Adamw", random_state = 42, **args):
    ...
    model, speed = train_model(model, self.loss,
                         ...
                         x_val, t_val, e_val, cuda = self.cuda == 2, patience_max = self.patience,
                         **args)
  ...
  def predict_survival(self, x, t, risk = 1, samples=1):
    ...
    if self.fitted:
      ...
      for t_ in t:
        ...
        log_sr, _, _, _ = self.torch_model(x, t_, sample=True, n_samples=samples)

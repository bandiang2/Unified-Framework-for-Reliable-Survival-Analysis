...
from torch.distributions import Bernoulli, Exponential, kl_divergence, Normal
...
class BayesianLinear(nn.Module):
    ...
    def __init__(
            ...
            config,
            positive = False,
            use_mixture: bool = True # True default
    ):
        super().__init__()
    ...
    def forward(
        ...
        if self.positive:
            return torch.einsum('bij,bkj->bik', x, weight.pow(2)) + bias** 2
    ...
    def kl_ib_output(self):
        ...
        kl_out = (- torch.log(sig_out) + 0.5 * (sig_out.pow(2) + mu_out.pow(2)) - 0.5).mean() #besst result with mean
    ...
    def reset_parameters(self):
        ...
        self.weight_mu.data.abs_().sqrt_()
...
class BayesianBasicBlock(nn.Module):
    ...
    def lwta_activation(self, input, temp = 0.67, hard = False, training = True):
        ...
        if self.training:
            ...
            kl = torch.mean(kl) #/ 1000. BEST RESULTS
...
class BayesianSparseNeuralSurvivalTorch(nn.Module):
    def __init__(self, input_dim, layers, config, output_dim=1, mlayers=1):
        ...
        if mlayers > 1:
            # self.monoLayers = nn.ModuleList()
            # for _ in range(mlayers):
            #     self.monoLayers(
            #         BayesianLinear(self.last_hidden_dim, self.last_hidden_dim, config, positive=True)
            #         )
            self.ml1 = BayesianLinear(self.last_hidden_dim, self.last_hidden_dim, config, positive=True)
            self.ml2 = BayesianLinear(self.last_hidden_dim, self.last_hidden_dim, config, positive=True)
        self.predictor = BayesianLinear(self.last_hidden_dim + 1, self.output_dim, config, positive=True)
        # Need all values positive 
        self.out_act = nn.Softplus()# nn.Sigmoid()
    ...
    def forward(self, x, horizon, sample=False, n_samples=1):
        ...
        for layer in self.layers:
            ...
            kl_total += layer.loss()
        # print(f"x after lwta layer: {x.shape}")
            
        if self.mlayers > 1:
            # for layer in self.monoLayers:
            #     x = layer(x, sample=sample, n_samples=n_samples)
            x = torch.tanh(self.ml1(x, sample=sample, n_samples=n_samples))
            kl_total += self.ml1.log_variational_posterior - self.ml1.log_prior
            x = torch.tanh(self.ml2(x, sample=sample, n_samples=n_samples))
            x = x.mean(dim=0)
            kl_total += self.ml2.log_variational_posterior - self.ml2.log_prior
                
        # information bound
        ib = 0.0
        if self.training:
            if self.mlayers > 1:
                ib += self.ml2.kl_ib_output()
            else:
        ...
        # print(f"x after mono layer: {x.shape}")
        output = self.predictor(torch.cat((x, tau_outcome), 1), sample=sample, n_samples=n_samples)
        output = output.mean(0)
        outcome = tau_outcome * self.out_act(output)

from tools.data_loader import (BaseDataLoader, FlchainDataLoader, GbsgDataLoader, MetabricDataLoader,
                               SupportDataLoader, WhasDataLoader, AidsDataLoader, SeerDataLoader, MimicDataLoader, TcgaDataLoader, TcgaDataLoader2, SyntheticDataLoader, NacdDataLoader)
...
def get_data_loader(dataset_name:str, ctype=None) -> BaseDataLoader:
    if dataset_name == "FLCHAIN":
        ...
        if ctype is not None:
            return TcgaDataLoader(type=ctype)
    elif dataset_name == "TCGA2":
        return TcgaDataLoader2()
    else:
        raise ValueError("Data loader not found")

def scale_data(X_train, X_valid=None, X_test=None, cat_features=[], num_features=[]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    return (X_train, X_valid, X_test) if X_test is not None else X_train
